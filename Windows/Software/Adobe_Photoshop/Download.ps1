...
$Source = Invoke-WebRequest -UseBasicParsing -Uri 'https://w16.monkrus.ws/search/label/Photoshop' | Select-Object -ExpandProperty 'Links' | Where-Object { $_.outerHTML -notmatch '#more' -and $_.outerHTML -notmatch 'Elements' -and $_.outerHTML -notmatch 'Collection' -and $_.outerHTML -match 'Multilingual' -and $_.outerHTML -match 'Photoshop' }
...
if ($Form.ShowDialog() -eq [Windows.Forms.DialogResult]::OK) {
    ...
    while (-not ($Directory = Get-ChildItem $env:TEMP -Directory -Filter '*Photoshop*' | Select-Object -First 1 | Select-Object -ExpandProperty 'FullName')) {
    ...
    do {
        Start-Sleep -Milliseconds 1000
    } until ((Get-Content $Log -ErrorAction SilentlyContinue) -match 'Torrent removed. Torrent: .*Photoshop*')
