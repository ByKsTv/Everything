...
$Source = (Invoke-WebRequest -UseBasicParsing -Uri 'https://w16.monkrus.ws/search/label/Photoshop').Links | Where-Object { $_.outerHTML -notmatch '#more' -and $_.outerHTML -notmatch 'Elements' -and $_.outerHTML -notmatch 'Collection' -and $_.outerHTML -match 'Multilingual' -and $_.outerHTML -match 'Photoshop' }
...
if ($Form.ShowDialog() -eq [Windows.Forms.DialogResult]::OK) {
    ...
    while (-not ($Directory = (Get-ChildItem $env:TEMP -Directory -Filter '*Photoshop*' | Select-Object -First 1).FullName)) {
