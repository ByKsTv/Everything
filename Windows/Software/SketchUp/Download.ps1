...
$Source = Invoke-WebRequest -UseBasicParsing -Uri 'https://nnmclub.to/forum/tracker.php?nm=SketchUp%20KpoJIuK' | Select-Object -ExpandProperty 'Links' | Where-Object { $_.class -match 'genmed topictitle' }
...
if ($Form.ShowDialog() -eq [Windows.Forms.DialogResult]::OK) {
    ...
    $Magnet = [Uri]::UnescapeDataString((Invoke-WebRequest -UseBasicParsing -Uri $TitleHREF | Select-Object -ExpandProperty 'Links' | Where-Object { $_.outerHTML -match 'magnet' } | Select-Object -First 1 | Select-Object -ExpandProperty 'href'))
    ...
    while (-not ($TempDir = Get-ChildItem $env:TEMP -Directory -Filter '*SketchUp*' | Select-Object -First 1 | Select-Object -ExpandProperty 'FullName')) {
    ...
    do {
        Start-Sleep -Milliseconds 1000
    } until ((Get-Content $Log -ErrorAction SilentlyContinue) -match 'Torrent removed. Torrent: .*SketchUp*')
