...
$Source = (Invoke-WebRequest -UseBasicParsing -Uri 'https://nnmclub.to/forum/tracker.php?nm=eM%20Client').Links | Where-Object { $_.class -match 'genmed topictitle' }
...
if ($Form.ShowDialog() -eq [Windows.Forms.DialogResult]::OK) {
    ...
    Remove-Item -Path "$env:TEMP\*eM Client*" -Force -Recurse -Confirm:$false -ErrorAction SilentlyContinue
    ...
    while (-not ($TempDir = (Get-ChildItem $env:TEMP -Directory -Filter '*eM Client*' | Select-Object -First 1).FullName)) {
