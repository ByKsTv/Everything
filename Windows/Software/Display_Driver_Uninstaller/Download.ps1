$DDU_DDL1 = (Invoke-WebRequest -UseBasicParsing -Uri 'https://www.wagnardsoft.com/display-driver-uninstaller-DDU-').Links | Where-Object { $_.outerHTML -match 'Download Display Driver Uninstaller' } | Select-Object -First 1 -ExpandProperty href
$DDU_DDL2 = (Invoke-WebRequest -UseBasicParsing -Uri ('https://www.wagnardsoft.com' + $DDU_DDL1)).Links | Where-Object { $_.outerHTML -match 'DOWNLOAD' } | Select-Object -First 1 -ExpandProperty href
$DDU_DDL = (Invoke-WebRequest -UseBasicParsing -Uri $DDU_DDL2).Links | Where-Object { $_.outerHTML -match 'setup.exe' } | Select-Object -First 1 -ExpandProperty href
