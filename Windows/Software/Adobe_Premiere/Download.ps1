...
$Form = New-Object System.Windows.Forms.Form -Property @{
    Text            = 'Adobe Premiere Pro Selection'
...
$Source = (Invoke-WebRequest -UseBasicParsing -Uri "$((Invoke-WebRequest 'http://www.monkrus.ws/' -UseBasicParsing -MaximumRedirection 0 -ErrorAction SilentlyContinue).Headers.Location)search/label/Premiere").Links | Where-Object { $_.outerHTML -notmatch '#comment-form' -and $_.outerHTML -notmatch '#more' -and $_.outerHTML -notmatch 'Collection' -and $_.outerHTML -notmatch 'Elements' -and $_.outerHTML -match 'Multilingual' -and $_.outerHTML -match 'Premiere' }
...
if ($Form.ShowDialog() -eq [Windows.Forms.DialogResult]::OK) {
    ...
    Remove-Item -Path "$env:TEMP\*Premiere*" -Force -Recurse -Confirm:$false -ErrorAction SilentlyContinue
    ...
    while (-not ($Directory = (Get-ChildItem $env:TEMP -Directory -Filter '*Premiere*' | Select-Object -First 1).FullName)) {
    ...
    do {
        Start-Sleep -Milliseconds 1000
    } until ((Get-Content $Log -ErrorAction SilentlyContinue) -match 'Torrent download finished. Torrent: .*Premiere*')
