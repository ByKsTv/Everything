...
$InstallPath = Split-Path ((Get-ItemProperty 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\mpv.exe' -ErrorAction SilentlyContinue).'(default)')
$UpdaterBAT = [IO.Path]::Combine($InstallPath, 'updater.bat')
