...
$Source = Invoke-WebRequest -UseBasicParsing -Uri 'https://w16.monkrus.ws/search/label/AutoCAD' | Select-Object -ExpandProperty 'Links' | Where-Object { $_.outerHTML -notmatch '#more' -and $_.outerHTML -match 'AutoCAD' -and $_.outerHTML -notmatch 'LT' -and $_.outerHTML -notmatch 'Addon' }
...
if ($Form.ShowDialog() -eq [Windows.Forms.DialogResult]::OK) {
    ...
    while (-not ($Directory = Get-ChildItem $env:TEMP -Directory -Filter '*AutoCAD*' | Select-Object -First 1 | Select-Object -ExpandProperty 'FullName')) {
