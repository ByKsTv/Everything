...
$Form = New-Object System.Windows.Forms.Form -Property @{
    Text            = 'Adobe After Effects Selection'
...
$Source = (Invoke-WebRequest -UseBasicParsing -Uri "$((Invoke-WebRequest 'http://www.monkrus.ws/' -UseBasicParsing -MaximumRedirection 0 -ErrorAction SilentlyContinue).Headers.Location)search/label/Effects").Links | Where-Object { $_.outerHTML -notmatch '#comment-form' -and $_.outerHTML -notmatch '#more' -and $_.outerHTML -match 'Multilingual' -and $_.outerHTML -match 'Effects' }
...
if ($Form.ShowDialog() -eq [Windows.Forms.DialogResult]::OK) {
    ...
    while (-not ($Directory = (Get-ChildItem $env:TEMP -Directory -Filter '*Effects*' | Select-Object -First 1).FullName)) {
