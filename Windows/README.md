...
1. Open NVCleanstall (Admin) > Connect Ethernet Cable > `Refresh` > `Manually select a driver version` > `Next` > `Recommended` > Uncheck `PhysX` > `Next` > `Use Previous Settings` > `Next` > `Install`.
