...
if (($null -eq $qBittorrent_InstalledVersion) -or ($qBittorrent_InstalledVersion -notmatch $qBittorrent_LatestVersion)) {
    ...
    $qBittorrent_SourceForge = ((Invoke-WebRequest -UseBasicParsing -Uri 'https://raw.githubusercontent.com/qbittorrent/qBittorrent-website/refs/heads/master/_site/download.html').Links | Where-Object { ($_.outerHTML -match 'sourceforge' -and $_.outerHTML -match '.exe' -and $_.outerHTML -match 'lt20' ) } | Select-Object -First 1).href
    ...
    [Console]::BackgroundColor = 'Black'; [Console]::ForegroundColor = 'Green'; [Console]::Write("Downloading 'qBittorrent' from "); [Console]::ForegroundColor = 'Yellow'; [Console]::Write("'$qBittorrent_DDL'"); [Console]::ForegroundColor = 'Green'; [Console]::Write(' to '); [Console]::ForegroundColor = 'Yellow'; [Console]::Write("'$qBittorrent_SavePath'"); [Console]::ResetColor(); [Console]::WriteLine()
    (New-Object System.Net.WebClient).DownloadFile($qBittorrent_DDL, $qBittorrent_SavePath)
    
    $qBittorrent_Argument = '/S'
    [Console]::BackgroundColor = 'Black'; [Console]::ForegroundColor = 'Green'; [Console]::Write("Installing 'qBittorrent' from "); [Console]::ForegroundColor = 'Yellow'; [Console]::Write("'$qBittorrent_SavePath'"); [Console]::ForegroundColor = 'Green'; [Console]::Write(' with '); [Console]::ForegroundColor = 'Yellow'; [Console]::Write("'$qBittorrent_Argument'"); [Console]::ResetColor(); [Console]::WriteLine()
