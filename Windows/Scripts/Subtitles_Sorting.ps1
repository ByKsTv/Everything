...
if ($FolderDialog.ShowDialog($Form) -eq [Windows.Forms.DialogResult]::OK) {
    ...
    Get-ChildItem $Subs_Path -Directory -Recurse | Sort-Object Name | ForEach-Object {
        $ENG_Subs = Get-ChildItem $_.FullName -File | Where-Object { $_.Name -match 'eng' -and $_.Extension -in '.srt', '.sub', '.idx', '.ass', '.sup' }
        if (-not $ENG_Subs) {
            $ENG_Subs = Get-ChildItem $_.FullName -File | Where-Object { $_.Name -match 'und' -and $_.Extension -in '.srt', '.sub', '.idx', '.ass', '.sup' }
        }
        if ($ENG_Subs) {
