...
$FileDialog.FileName = 'Select PLAYLIST Folder'

$form = New-Object System.Windows.Forms.Form
$form.TopMost = $true

if ($FileDialog.ShowDialog($form) -eq 'OK') {
    $testFolder = [IO.Path]::GetDirectoryName($FileDialog.FileName)
    Write-Output "Selected Folder: $testFolder"

    $InputForm = New-Object System.Windows.Forms.Form
    $InputForm.Text = 'Episode'
    $InputForm.Size = New-Object System.Drawing.Size(300, 150)
    $InputForm.StartPosition = 'CenterScreen'
    $InputForm.TopMost = $true

    $Label = New-Object System.Windows.Forms.Label
    $Label.Text = 'Starts from episode:'
    $Label.AutoSize = $true
    ...
    $ChosenNumber = $InputForm.Tag

    $ChosenNumber = [int]$ChosenNumber
    Write-Output "Starts from episode: $ChosenNumber"

    
    Write-Output "Searching for .mpls files in directory: $testFolder"

    # Get all .mpls files in the specified directory and subdirectories
    $mplsFiles = Get-ChildItem -Path $testFolder -Recurse -Include *.mpls

    # Check if any .mpls files were found
    if ($mplsFiles.Count -eq 0) {
        Write-Output "No .mpls files found in the specified directory: $testFolder"
        exit
    }

    Write-Output "Found $($mplsFiles.Count) .mpls files in the directory: $testFolder"
