...
if ($PCPasswordAnswer -eq 'Yes') {
	...
	if ($PCPasswordResult -eq [System.Windows.Forms.DialogResult]::OK) {
		...
		New-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon' -Name 'DefaultPassword' -Value $PCPasswordResultPASS -PropertyType String -Force
