...
function Get-Latest-Mpv($Arch) {
    ...
    $filename = $json.assets | Where-Object { $_.name -Match "mpv-$Arch-[0-9]{8}" } | Select-Object -ExpandProperty name
    $download_link = $json.assets | Where-Object { $_.name -Match "mpv-$Arch-[0-9]{8}" } | Select-Object -ExpandProperty browser_download_url
...
function Get-Latest-FFmpeg ($Arch) {
    $api_gh = 'https://api.github.com/repos/zhongfly/mpv-winbuild/releases/latest'
    $json = Invoke-WebRequest $api_gh -MaximumRedirection 0 -ErrorAction Ignore -UseBasicParsing | ConvertFrom-Json
    $filename = $json.assets | Where-Object { $_.name -Match "ffmpeg-$Arch-git-" } | Select-Object -ExpandProperty name
    $download_link = $json.assets | Where-Object { $_.name -Match "ffmpeg-$Arch-git-" } | Select-Object -ExpandProperty browser_download_url
    if ($filename -is [array]) {
        return $filename[0], $download_link[0]
    }
    else {
